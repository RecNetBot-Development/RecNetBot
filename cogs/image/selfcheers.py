...
async def selfcheers(
    ...
    if self_cheered_posts:
        image_ui_view, embeds = await ImageUI(ctx=ctx, user=user, posts=self_cheered_posts, interaction=ctx.interaction, original_embeds=[self_cheers_embed(ctx, user, self_cheers)]).start()
    else:
        embeds = [self_cheers_embed(ctx, user, self_cheers)]
    await ctx.respond(f"Time elapsed: `{round(end_time-start_time, 2)}s`", embeds=embeds, view=image_ui_view)