...
@slash_command(
    ...
    name="data",
    description="Fetch raw account data."
)
async def data(
    self, 
    ctx, 
    name: Option(str, "Enter account name or id", required=True),
    is_id: Option(bool, "If it's an id, specify it here. If unspecified, it will be assumed", required=False)
):
    await base_api(rec_net=self.bot.rec_net.rec_net, ctx=ctx, type="Account", name=name, is_id=is_id)