...
def resolve_warnings(x: int):
    warnings = [None, "Spooky/scary themes", "Mature themes", "Bright/flashing lights", "Intense motion", "Gore/violence", "Custom"]
...
class Room(BaseDataclass):
    ...
    async def from_data(cls, data):
        if isinstance(data, list): return [*map(Room.from_data, data)]
        supported_platforms = []
        ...
        await room.patch_info(data=data)
        return room